...
import User from "../model/userMod";
...
describe("Blog Routes", () => {
  ...
  beforeAll(async () => {
    await mongoose.connect(
      "mongodb+srv://user1:user1@cluster0.q3w70mq.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
    );
    // await mongoose.connect(mongoServer.getUri());

    //register dummy user
    const dummyUserRegister = await request(app)
    ...
    const registerDummyAdmin = await request(app)
      .post("/api/v1/users/auth/register")
      .send(dummyAdmin2);
  ...
  describe("GET /api/v1/blogs/:id", () => {
    test("spying on getSingleBlog service", async () => {
      ...
      const response = await request(app).get(`/api/v1/blogs/${testBlogId}`);
      expect(spy).toHaveBeenCalled();
    }, 20000);

    test("should return 400 if blog invalid id passed", async () => {
      const spy = jest.spyOn(blogService, "getBlogById");
      const response = await request(app).get(`/api/v1/blogs/someidher`);
      expect(response.status).toBe(400);
      expect(spy).not.toHaveBeenCalled();
    }, 20000);
    test("should return 404 if blog id does not exist", async () => {
      const spy = jest.spyOn(blogService, "getBlogById");
      const response = await request(app).get(`/api/v1/blogs/${invalidId}`);

...
import User, { UserDocument } from "../model/userMod";
...
export const getAllUsers = async () => {
  try {
    ...
    throw new Error(
      `Error happened while fetching all users: ${error.message}`
    );
  }
};
...
export const getUsersCount = async () => {
  try {
    ...
    throw new Error(
      `Error happened while getting user counts ${error.message}`
    );
  }
};
